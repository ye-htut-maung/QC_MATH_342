...
df = data.frame (
  col1 = rnorm(100, mean = 17, sd = sqrt(38)),
  col2 = runif(100, min = -10, max = 10),
  col3 = rpois(100, lambda = 6),
  col4 = rexp(100, rate = 9),
  col5 = rbinom(100, size = 20, prob = 0.12),
  col6 = as.integer(1:100 %in% sample(100, size = 100 * 0.24, replace = FALSE))
  
)

rownames(df) = fake_first_names
...
```{r}
#TO-DO
df$col6 = factor(ifelse(df$col6==1, "FOREIGN", "DOMESTIC"))
...
for (i in 1 : n){
  currentThreshold = Xy$Sepal.Length[i]
  num_errors = sum((Xy$Sepal.Length >= currentThreshold) != y)
  num_errors_by_parameter[i, ] = c(currentThreshold, num_errors)
}
...
```{r}
#TO-DO
totalErrors = sum(ifelse(Xy$Sepal.Length >= threshold, 1, 0) != y)
totalErrors
...
It makes sense because the mean and median of versicolor is higher than threshold which is 5.5 and 1st quadrant is around 5.5. Also setosa 3rd quadrant is around 5.2 and it is below 5.5.
...
sepalLengthTest = sample(Xy$Sepal.Length, size = 1)
sepalLengthTest
g(sepalLengthTest)
```
