...
Nan is there for the log of negative numbers and -Inf is for log_3(0). It is expected and appropriate because of the negative numbers.
...
Numeric can store inf and -inf but integers cannot store. That's NA is showing instead of inf.
...
```{r}
#TO-DO
sum(seq(from=0, to=1, by=1e-6)^2) * 1e-6
```
...
```{r}
#TO-DO
mean(sample(0:1, size = 100, replace = TRUE))
```
...
```{r}
#TO-DO
mean(sample(0:1, size = 500, replace = TRUE, prob = c(0.1,0.9)))
```
...
#TO-DO
n = 50
R = matrix(sample(c(0,1,2), size = n*n, replace=TRUE, prob=c(0.5,0.25,0.25)),nrow = n, ncol = n)
head(R)
```
...
```{r}
#TO-DO
R[sample(c(TRUE, FALSE), size = n ^ 2, prob = c(0.30, 0.70), replace = TRUE)] = NA
head(R)
R[runif(n^2) < 0.3] = NA
head(R)
```
...
#TO-DO
?lapply
lapply(1:ncol(R), function(m) list(min = min(R[,m], na.rm=TRUE), max=max(R[,m], na.rm=TRUE), pct_missing = mean(is.na(R[,m])), first_NA = which(is.na(R[,m]))[1]) )
...
#TO-DO
set.seed(6666)
head(sample(rnorm(1000, mean = -10,sd=10 )))
set.seed(1234)
head(sample(rnorm(1000, mean = -10,sd=10 )))
```
...
abs(quantile(v, probs = 0.05) - qnorm(0.05, mean = -10, sd = 10))
# yes, since, the error is 0.367.. 
```
...
mean(v <= 0) # practical percentile of v
pnorm(0, mean = -10, sd= 10)

# yes the estimate is about what is expected by theory
...
```{r}
#TO-DO
my_reverse = function(vector_v) {
  reversed_vec = c(1:length(vector_v))
  reversed_vec[1:length(vector_v)] = vector_v[length(vector_v):1]
  reversed_vec
} 
...
```{r}
#TO-DO
flip_matrix = function(matrix_m, dim_to_rev = NULL) {
  if(is.null(dim_to_rev)) {
    dim_to_rev = ifelse(nrow(matrix_m) >= ncol(matrix_m), 1, 2)
  }
  if (!dim_to_rev %in% c(1,2)) {
    stop("dim_to_rev must be 1 for rows and 2 for columns")
  }
  if(dim_to_rev == 1) {
    return (matrix_m[nrow(matrix_m):1, , drop=FALSE])
  }
  if (dim_to_rev == 2) {
    return (matrix_m[, ncol(matrix_m):1, drop=FALSE])
  }
}
```
...
```{r}
#TO-DO
#my_list =list("A" = array(1),
#              "B" = array(1:4, c(2,2)),
#              "C" = array(1:9, c(3,3,3)),
#              "D" = array(1:16, c()) and so on
#              )
...
for (i in 1:8) {
  myletter = LETTERS[i]
  my_list[[myletter]] = array(1:i ^ i, dim = c(rep(i,i)))
}
my_list
...
#TO-DO
`?object.size` return an estimate of the memory that is being used to store and R object. So each element of the list has certain numbers of bytes corresponding to how many value they hold. It also makes sense given the dimensions of the arrays as the bytes increase greatly as the size is growing exponentially.
...
split_sentences = strsplit(lorem, split="[.] ")[[1]]
sample(split_sentences)
```
...
gender_generation = list(
  Male = list(
    Boomer = M_Boomer,
    GenX = M_GenX,
    Millennial = M_Millennial
  ),
  Female = list(
    Boomer = F_Boomer,
    GenX = F_GenX,
    Millennial = F_Millennial
  )
)
gender_generation
